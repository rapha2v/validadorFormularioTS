...
/*form and inputs*/
function validationUser(userClient) {
    hideErrorMessage(user);
    if (validator_1.default.isAlphanumeric(userClient))
        return true;
    if (userClient === '')
        showErrorMessage(user, 'Usuário em branco');
    else
        showErrorMessage(user, 'Usuário inválido. Permitido apenas carácteres alfanuméricos.');
    return false;
}
function validationEmail(emailClient) {
    hideErrorMessage(email);
    if (validator_1.default.isEmail(emailClient))
        return true;
    if (emailClient === '')
        showErrorMessage(email, 'Email em branco');
    else
        showErrorMessage(email, 'Email inválido');
    return false;
}
function validationPassword(pass, confirmPass) {
...
form.addEventListener('submit', (e) => {
    ...
    const validatedPassword = !validationPassword(password.value, confirmPassword.value);
